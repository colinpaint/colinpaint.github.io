...
mat3 rotation_matrix (vec3 a, float angle) {
  ...
  float sub_c = 1.0 - c;
  float s = sin (angle);
  return mat3 (vec3 (c + pow(a.x, 2.0) * sub_c,    a.y * a.x * sub_c + a.z * s,  a.z * a.x * sub_c - a.y * s),
	       vec3 (a.x * a.y * sub_c - a.z * s,  c + pow(a.y, 2.0) * sub_c,    a.z * a.y * sub_c + a.x * s),
	       vec3 (a.x * a.z * sub_c + a.y * s,  a.y * a.z * sub_c - a.x * s,  c + pow(a.z, 2.0) * sub_c));
  }
`
}}}
{{{
...
void main() {
  ...
  if (len >= 0.95 || len <= 0.85) {
    float luminance = 0.2126 * brush_color.r + 0.7152 * brush_color.g + 0.0722 * brush_color.b;
    if (luminance > 0.2) 
      color.rgb = vec3 (0.0);
    else 
     color.rgb = vec3 (1.0);
